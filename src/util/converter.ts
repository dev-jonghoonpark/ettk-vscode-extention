...
function engTypeToKor(src: string) {
  let res = "";
  if (src.length === 0) {
    return res;
  }
...
function makeHangul(nCho: number, nJung: number, nJong: number) {
  return String.fromCharCode(0xac00 + nCho * 21 * 28 + nJung * 28 + nJong + 1);
}

function korTypeToEng(src: string) {
  let res = "";
  if (src.length === 0) {
    return res;
  }
...
export { engTypeToKor, korTypeToEng };
